...
hh.unlabel <- hh.unlabel %>% select(scale_hhid, scale_pid, hhid, date, cluster, age, sex, room, sroom,
                                    gas, electric, firewood, charcoal, kerosene, diesel, indoor, vent, settle, nlive, nleft, ndied, smoke)
write.csv(hh.unlabel, here("data", "hh.unlabel.csv"))
...
write.csv(hh.labeled, here("data", "hh.labeled.csv"))

#=================================================================================

#load participant dataset without labels
pp.unlabel <- read_csv(here::here("data", "participant.csv"))

pp.unlabel <- select(rename(pp.unlabel,
"sdate" = data_date, "scale_pid" = s1_2, "somipa_hhid" = s4_1, "pno" = s4_2, "somipa_pid" = pid, "pstatus" = s4_3, 
...
sdate, scale_pid, somipa_hhid, pno, somipa_pid, pstatus, dob, age, agescale, agedeter, sex, occup, educ, dtravel, pvisit, vtime, transfot, transbic, transcar, transpub, transbac, transdk, cvdcnt, cvdno, cvdhome, cvdwork, cvdxool, cvdchurch, cvdmrkt, cvdothr, cvdinfant, cvdprexool, cvdprixool, cvdsecxool, cvdadult, cvdelderly, cntno)
...
scale_pid, somipa_hhid, pno, somipa_pid, sdate, pstatus, agey, agexact, sex, occup, educ, dtravel, pvisit, vtime, transfot, transbic, transcar, transpub, transbac, transdk, cvdcnt, cvdno, cvdhome, cvdwork, cvdxool, cvdchurch, cvdmrkt, cvdothr, cvdinfant, cvdprexool, cvdprixool, cvdsecxool, cvdadult, cvdelderly, cntno)
...
scale.hivq <- read_csv(here::here("data", "scalehiv.csv"))
pp.unlabel <- left_join(pp.unlabel, scale.hivq, by = "scale_pid")

write.csv(pp.unlabel, here("data", "pp.unlabel.csv"))
...
#effect of COVID-19
hiv = if_else(hiv == 2, "Positive on ART", if_else(hiv == 3, "Negative", NA_character_)))

write.csv(pp.labeled, here("data", "pp.labeled.csv"))
...
"cnt_age" = s6_1_2a, "cnt_scale" = s6_1_2b, "cnt_sex" = s6_1_3, "cnt_type" = s6_1_4, "cnt_rel" = s6_1_5, "cnt_loc" = s6_1_6, "cnt_plc" = s6_1_7, "cnt_freq" = s6_1_9, "cnt_dur" = s6_1_10, "eplon" = EPAL_LOCATION_LONGITUDE, "eplat" = EPAL_LOCATION_LATITUDE), 
sdate, edate, iid, scale_pid, somipa_hhid, pno, somipa_pid, part_sex, cnt_no, cnt_age, cnt_scale, cnt_sex, cnt_type, cnt_rel, cnt_loc, cnt_plc, cnt_freq, cnt_dur, eplon, eplat)

contacts.Q <- select(contacts.Q %>% mutate(cnt_age = if_else(cnt_scale == 1, cnt_age/12, cnt_age)),
                   sdate, edate, iid, scale_pid, somipa_hhid, pno, somipa_pid, part_sex, cnt_no, cnt_age, cnt_sex, cnt_type, cnt_rel, cnt_loc, cnt_plc, cnt_freq, cnt_dur, eplon, eplat)

cn.unlabel <- contacts.Q %>% filter(!is.na(scale_pid))

# merge somipa contacts with households and with scale
cn.unlabel <- cn.unlabel %>% rename("hhid" = somipa_hhid)
cn.unlabel <- left_join(cn.unlabel, select(hh.unlabel, hhid, scale_hhid), by = "hhid")

# load scale household coordinates dataset 
spatial <- read_csv(here::here("data", "coordinates_scale.csv"))
spatial <- distinct(spatial, hhid, .keep_all = TRUE)
spatial <- spatial %>% rename("scale_hhid" = hhid)
cn.unlabel <- left_join(cn.unlabel, spatial)

# calculate spatial distance between two coordinates
cn.unlabel <- cn.unlabel %>% mutate(eplon = if_else(eplon < 0, gps_lng, eplon), eplat = if_else(eplat < -20, gps_lat, eplat))
cn.unlabel <- cn.unlabel %>% mutate(gps_lng = if_else(is.na(gps_lng), eplon, gps_lng), gps_lat = if_else(is.na(gps_lat), eplat, gps_lat))
...
cn.unlabel$cnt_dist <- get_geo_distance(cn.unlabel$gps_lng, cn.unlabel$gps_lat, cn.unlabel$eplon, cn.unlabel$eplat)
cn.unlabel <- cn.unlabel %>% mutate(cnt_dist = if_else(cnt_dist == 0, NA_real_, cnt_dist))
cn.unlabel <- cn.unlabel %>% select(edate, hhid, pno, somipa_pid, part_sex, cnt_no, cnt_age, cnt_sex, cnt_type, cnt_rel, cnt_loc, cnt_plc, cnt_freq, cnt_dur, cnt_dist)

write.csv(cn.unlabel, here("data", "cn.unlabel.csv"))
...
write.csv(cn.labeled, here("data", "cn.labeled.csv"))
